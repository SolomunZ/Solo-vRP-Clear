...
--        print ("O id "..user_id.." passou pelo Creator!")
        ...
        TriggerEvent("Nikito:resetdimensao")
        Wait(1000)
...
function processSpawnController(source,char,user_id)
	...
	if char then
        ...
        if data and data ~= "" then 
            ...
            TriggerEvent("Nikito:setardimensao")
        end
	end
end


RegisterServerEvent("Nikito:setardimensao")
AddEventHandler("Nikito:setardimensao",function(id)
    local source = source
    local user_id = id or vRP.getUserId(source)
    if user_id then
...
RegisterServerEvent("Nikito:resetdimensao")
AddEventHandler("Nikito:resetdimensao",function()
	local source = source
    local user_id = id or vRP.getUserId(source)
    if user_id then

...
AddEventHandler("baseModule:idLoaded",function(source,user_id,model,name,firstname,age)
	...
	if vRP.rusers[user_id] == nil then
		...
		if name ~= nil and firstname ~= nil then
			...
			vRP.giveInventoryItem(user_id,"dinheiro",configs.moneyWallet)
		end
	...
	if steam then
		...
		if not vRP.isBanned(steam) then
			if vRP.isWhitelisted(steam) then
				...
				deferrals.done(configs['msg_wl']..tostring(id or tonumber(newUser[1].id) or steam))
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
			deferrals.done(configs['msg_banido'])
			TriggerEvent("queue:playerConnectingRemoveQueues",ids)
		end
	else
		deferrals.done(configs['msg_steam'])
		TriggerEvent("queue:playerConnectingRemoveQueues",ids)
	end
end)

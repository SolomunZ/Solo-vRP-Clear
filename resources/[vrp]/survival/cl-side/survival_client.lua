...
vRPclient = Tunnel.getInterface("vRP","survival")
--[SURVIVAL TUNNEL/PROXY]--
Survival = {}
Pserver = Tunnel.getInterface("survival","survival")
SurvivalSERVER = Tunnel.getInterface("survival")
Tunnel.bindInterface("survival",Survival)


vSERVER = Tunnel.getInterface(GetCurrentResourceName())

cutscene_desistencia = survival.cutscene_pos_desistir
cutscene = survival.cutscene_pos_morte
localrenacer = survival.respawn_pos_morte
localdesistir = survival.respawn_pos_desistir
DeathTime = survival.tempo_morte 

direct_do_ped_p_m =  survival.heading_do_ped_p_m
direct_do_ped_p_d =  survival.heading_do_ped_p_d

ter_primeira_vis = survival.prim_vis
-----------------------------------------------------------------------------------------------------------------------------------------
--[ INDICAR NOCAUTEADO ]-----------------------------------------------------------------------------------------------------------------
--------[ THREAD ]-----------------------------------------------------------------------------------------------------------------------
...
RegisterNUICallback('Survival:BotaoReviver',function()
	...
	TriggerEvent("Survival:ForceNuiOff") ---- DESLIGAR NUI ANTES DA CUTSCENE
	if quero_cutscene == true then
		TriggerEvent("Survival:Cutscene")
	end
	deathtimer = DeathTime
	nocauteado = false
...
RegisterNUICallback('Survival:BotaoDesistir',function()
	local ped = PlayerPedId()
	local timer = SurvivalSERVER.timedesistencia()
	if SurvivalSERVER.checkTimeDesistir() then
		deathtimer = DeathTime
		nocauteado = false
		SetNuiFocus(false,false)
		TransitionFromBlurred(1000)
		...
		TriggerEvent("Survival:desistir")
		TriggerEvent("resetBleeding")
		TriggerEvent("resetDiagnostic")
		SetFollowPedCamViewMode(1)
		...
		SetTimeout(5000,function()
			SurvivalSERVER.SetTimeDesistir()
			FreezeEntityPosition(ped,false)
			Citizen.Wait(1000)
			DoScreenFadeIn(1000)
			ShakeGameplayCam("LARGE_EXPLOSION_SHAKE",0.03)
...
--	TriggerEvent("survival:Notify","negado","Você não podera desistir de sua Vida agora! Aguarde "..timer.." Segundos !")
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(750)
		if SurvivalSERVER.checkuserid() then
			local timer = SurvivalSERVER.timedesistencia()
			if timer > 0 then
				SurvivalSERVER.diminuirtimedesistencia()
			end
		end
	end
end)
...
RegisterNetEvent("Survival:NuiOff")
AddEventHandler("Survival:NuiOff",function()
	local ped = PlayerPedId()
	TriggerEvent("resetBleeding")
	TriggerEvent("resetDiagnostic")
	deathtimer = DeathTime
...
RegisterNetEvent("Survival:ForceNuiOff")
AddEventHandler("Survival:ForceNuiOff",function()
	deathtimer = 0
	nocauteado = false
	SetNuiFocus(false,false)
	TransitionFromBlurred(1000)
	TriggerEvent("Survival:NuiOff")
	SendNUIMessage({
		setDisplay = false,
		setDisplayDead = false,
		deathtimer = deathtimer
...
--[ VOLTAR A VIDA ]----------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
local survivalinvivo = false
RegisterNetEvent("Survival:life")
AddEventHandler("Survival:life",function()
    local ped = PlayerPedId()
	...
	SetTimeout(1000,function()
		...
		TriggerEvent("Survival:ForceNuiOff") --- PRIMEIRA TENTATIVA DE DESLIGAR NUI
...
RegisterNetEvent("Survival:desistir")
AddEventHandler("Survival:desistir",function()
    local ped = PlayerPedId()
    local health = GetEntityHealth(ped)
    local armour = GetPedArmour(ped)
...
SurvivalSceneDeath = 'MP_INT_MCS_18_A1' 

RegisterNetEvent("Survival:Cutscene")
AddEventHandler("Survival:Cutscene",function()
	local ped = PlayerPedId()
    ...
    SetRainLevel(0.0)
    PlayCutscene(SurvivalSceneDeath, vector3(-1314.997, -1721.084, 1.1493))
...
RegisterNetEvent('survival:Anim')
AddEventHandler('survival:Anim', function()
